    ...
    def __init__(
        ...
        for l in range(self.layer_num):
            if l == 0:
                ...
                self.wm[f"fc{l}"] = nn.Parameter(torch.zeros(input_dim, hidden_dims[l], requires_grad=True))
                self.bn.append(nn.BatchNorm1d(hidden_dims[l]))

            elif l == self.layer_num - 1:
                self.fc.append(nn.Linear(hidden_dims[l - 1], output_dim))
                self.wm[f"fc{l}"] = nn.Parameter(torch.zeros(hidden_dims[l - 1], output_dim, requires_grad=True))
                ...
                self.wm[f"fc{l}"] = nn.Parameter(torch.zeros(hidden_dims[l - 1], hidden_dims[l], requires_grad=True))
                self.bn.append(nn.BatchNorm1d(hidden_dims[l]))
