    ...
    for module_idx, module in enumerate(backbone.modules()):
        if isinstance(module, nn.Conv2d) or isinstance(module, nn.Linear):
            ...
            if module.weight.grad is not None:
                module.weight.grad.data[layer_mask.ne(1)] = 0
...
def pruning_mask(weights, union_mask, layer_idx, prune_perc):
    ...
    remove_mask = weights.abs().le(cutoff_value) * union_mask.eq(1)
...
def prune(backbone: nn.Module, union_mask, task_id, prune_perc):
    ...
    print("Pruning each layer by removing %.2f%% of values" % (100 * prune_perc))
    for module_idx, module in enumerate(backbone.modules()):
        if isinstance(module, nn.Conv2d) or isinstance(module, nn.Linear):
            mask = pruning_mask(
